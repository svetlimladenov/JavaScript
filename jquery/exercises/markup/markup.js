const parse = function parse(selector) {
        ...
        if (isOpening) {
            ...
            if (indexOfEndTag > -1) {
                ...
                if (indexOfSeparator > -1) {
                    ...
                    linkUrl = secondPart.slice(0, indexOfSeparator - 1);
                }
            }
            
        }
        const replaceTag = isOpening ? `<${replaceString} href='${linkUrl}'> ${linkText}` : `</${replaceString}>`; // ako e opening togava slagai href

        return `${firstPart}${replaceTag}`;
    }
    ...
    function replace(line, openingSymbol, closingSymbol, replaceString, newStringAction) {
        ...
        if(index > -1) {
            let editedLine = newStringAction(index, line, openingSymbol, closingSymbol, replaceString, true);
